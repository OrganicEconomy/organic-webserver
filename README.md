...
- this webserver (a REST API in Nodejs + express + TypeScript);
- a SQLite database by default (Postgres available via `DB_DIALECT=postgres`);
- a webapp (in Angular).

## Production

Create a .env file at the root of the folder (see `.env.dev` for the variables).

The server speaks plain HTTP: TLS is terminated by a reverse proxy placed in
front (Caddy). Deployment recipes — Caddy, Docker, Postgres — live in the
[organic-deploy](https://github.com/OrganicEconomy/organic-deploy) repository.
...
Docker recipes have moved to the
[organic-deploy](https://github.com/OrganicEconomy/organic-deploy) repository.
...
curl -i -X POST -H 'Content-Type: application/json' \
  ...
  http://127.0.0.1:6868/api/users/register
```
...
curl -i -X POST -H 'Content-Type: application/json' \
  ...
  http://127.0.0.1:6868/api/users/login
```
...
curl -i -X PUT -H 'Content-Type: application/json' \
  ...
  http://127.0.0.1:6868/api/users/save
```
...
curl -i -X PUT -H 'Content-Type: application/json' \
  ...
  \
  -d '{"publickey":"02c85e4e448d67a8dc724c620f3fe7d2a3a3cce9fe905b918f712396b4f8effcb3","block":{"v":1,"d":99991231,"p":"3045022100f1f99ced8b08b07e6968e691087ee6d146e5829d89a557f2f0db0e8707bf4a2a02201478dce4233405baf5467ec7c81fd514fb5ffc69df3ba34967bbf344771becb4","s":null,"r":0,"m":[],"i":[202511259000],"t":0,"h":null,"x":[{"d":20260604,"m":[20251125000],"i":[],"s":"02c85e4e448d67a8dc724c620f3fe7d2a3a3cce9fe905b918f712396b4f8effcb3","t":3,"p":"0306ffd8f4fe843f5f7183179dcf36f550326813f56ec824911abca9c9d1cd7834","v":1,"h":"3044022011670c09cbd20f35e35dbf8d532162b101813317390c2a408d65d081a2fc6ebd022039dcacce321b146224f9ccb028b072707eec02b7726c18a8bc0b9aaff474b90c"}]}}' \
  http://127.0.0.1:6868/api/users/sign
```
...
curl -i -X POST -H 'Content-Type: application/json' \
  ...
  http://127.0.0.1:6868/api/tx/send
```
...
curl --get \
  ...
  -d "timestamp=$TS" \
  http://127.0.0.1:6868/api/tx/list
```
